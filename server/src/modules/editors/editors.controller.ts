...
export class EditorsController {
  ...
  async create(@Body() body: { name: string; price: string }) {
    const result = await this.editorsService.create(body.name, body.price)
    return {
      code: 200,
      message: '创建成功',
      data: result
  ...
  @Put(':id')
  async update(@Param('id') id: number, @Body() body: { name?: string; price?: string; is_active?: boolean }) {
